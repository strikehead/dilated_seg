...
def get_frontend(input_width, input_height) -> Sequential:
    ...
    model.add(MaxPooling2D((2, 2), strides=(2, 2)))  # 248,248,64

    model.add(Convolution2D(128, 3, 3, activation='relu', name='conv2_1'))
    model.add(Convolution2D(128, 3, 3, activation='relu', name='conv2_2'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2))) # 122, 122, 128

    model.add(Convolution2D(256, 3, 3, activation='relu', name='conv3_1'))
    model.add(Convolution2D(256, 3, 3, activation='relu', name='conv3_2'))
    model.add(Convolution2D(256, 3, 3, activation='relu', name='conv3_3'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2))) # 58, 58, 256

    model.add(Convolution2D(512, 3, 3, activation='relu', name='conv4_1')) # -2
    model.add(Convolution2D(512, 3, 3, activation='relu', name='conv4_2')) # -2
    model.add(Convolution2D(512, 3, 3, activation='relu', name='conv4_3'))  # 52, 52, 512

    # Compared to the original VGG16, we skip the next 2 MaxPool layers,
    # and go ahead with dilated convolutional layers instead

    model.add(AtrousConvolution2D(512, 3, 3, atrous_rate=(2, 2), activation='relu', name='conv5_1')) # -4
    model.add(AtrousConvolution2D(512, 3, 3, atrous_rate=(2, 2), activation='relu', name='conv5_2')) # -4
    model.add(AtrousConvolution2D(512, 3, 3, atrous_rate=(2, 2), activation='relu', name='conv5_3'))  # 40, 40, 512

    # Compared to the VGG16, we replace the FC layer with a convolution

    model.add(AtrousConvolution2D(4096, 7, 7, atrous_rate=(4, 4), activation='relu', name='fc6')) # -(3*4) -12 = 16, 16, 512
